...
const form = useForm({
    ...
    images: props.specialFeature?.images ?? [],
    status: props.specialFeature?.status ?? "",
});

const blogHandle = () => {
    let url = "";
    if (props.specialFeature?.id) {
        url = route("admin.special-feature.update", props.specialFeature.id);
    } else {
        url = route("admin.special-feature.store");
    }
        ...
        <form @submit.prevent="blogHandle" class="mt-6 space-y-6 p-3">
            ...
            <div class="flex items-center justify-end pr-5 py-5">
                <PrimaryButton :disabled="form.processing">
                    {{ specialFeature?.id ? "Update" : "Submit" }}
                </PrimaryButton>
